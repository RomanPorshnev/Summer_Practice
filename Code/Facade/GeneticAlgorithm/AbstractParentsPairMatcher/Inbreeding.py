...
class Inbreeding(AbstractParentsMatcher):
    '''
    Данный метод формирует и возвращает пары родителей, сформированные по принципу инбридинга
    ...
    def make_parents_pairs(self):
        while len(self._parents) != 1:
            ...
            min_hamming_distance = len(first_parent)
            index_of_second_parent = 0
            for i in range(len(self._parents)):
                hamming_distance = self._hamming_distance(first_parent, self._parents[i])
                if hamming_distance <= min_hamming_distance and i != index_of_first_parent:
